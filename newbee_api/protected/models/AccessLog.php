...
class AccessLog extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('user_id, controller, action, date, add_time', 'required'),
			array('user_id, controller, action, date, add_time', 'length', 'max'=>10),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, user_id, controller, action, date, add_time', 'safe'),
		);
	}
