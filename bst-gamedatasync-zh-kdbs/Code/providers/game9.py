...
class game9(providerBase.providerBase):
    ...
    #}
        ...
        if package.get("downUrl",None):
            data['download_url'] = package['downUrl']
        if package.get("extendInfo",None):
            try:
                data['package_name'] = package['extendInfo']['packageName']
    ...
    def getData(self,syncId,pageNum,pageSize,starttime,endtime):
        ...
        # 需要同步的数据字段
        data["syncField"] = "game.id,game.name,game.categoryId,platform.logoImageUrl,package.fileSize,platform.screenshotImageUrls,platform.version,game.keywords,platform.score,platform.instruction,platform.description,package.downUrl,package.packageName,package.extendInfo"
