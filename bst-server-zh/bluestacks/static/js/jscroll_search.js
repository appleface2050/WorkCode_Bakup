...
var objs;
var scrolls = function (a,obj){
	objs=obj;
	var tur=true;
	var self = this;
	var timer;
	var window_h=$(window).height();
    var appcenter_tab_h=$(".appcenter_tab").height();
   // console.log(window_h+"_"+appcenter_tab_h)
	this.container = $I(a+"_container"); // 容器
   // console.log(window_h);
	//self.container.offsetHeight=window_h-appcenter_tab_h-8;//自己加
	...
	this.bar_is_bottom = function(){ // 检测是不是位于底部了
		if (self.scroll_bar.offsetTop >= self.scroller.offsetHeight-self.scroll_bar.offsetHeight){
			return true;
		}
		return false;
	}
	/*this.init=function(){
		//console.log(self.container.offsetHeight);
		self.container.offsetHeight=window_h-appcenter_tab_h-8;//自己加
	console.log(self.container.offsetHeight);
	...
	this.resizesearch=function(){
		//alert(33);
		bar_top=self.scroll_bar.style.top;
		bar_leave=self.scroller.offsetHeight - self.scroll_down.offsetHeight - self.scroll_bar.offsetHeight - self.scroll_up.offsetHeight;
		if(bar_top.split("p")[0]>bar_leave){
			self.scroll_bar.style.top=bar_leave+"px";
		}
			
		shower_top=self.shower.style.top;
		shower_leave=self.shower.offsetHeight - self.container.offsetHeight;	
		if(-shower_top.split("p")[0]>shower_leave){
			self.shower.style.top=-shower_leave+"px";
		}	
	...
	this.move=function(a){
		var mosuse_move_length;
		if(self.shower.offsetTop <(self.container.offsetHeight-self.shower.offsetHeight)){
			mosuse_move_length=0;
		}else{
			mosuse_move_length=a;
		}
		if (self.shower.offsetTop+mosuse_move_length <= 0 && self.shower.offsetTop+mosuse_move_length >= self.container.offsetHeight-self.shower.offsetHeight){
				self.shower.style.top = (self.shower.offsetTop+mosuse_move_length)+"px";
			}else if (self.shower.offsetTop+mosuse_move_length > 0){
				self.shower.style.top = 0+"px";
			}else if (self.shower.offsetTop+mosuse_move_length <= self.container.offsetHeight-self.shower.offsetHeight){
	...
	this.loading_left=function (){
		if($(".jzz span").length>0){
			if(index<=total_page){
				getsearch(objs);
		...
		this.drag = function(){
			//$(document).mouseup(function(){
				if (self.container.offsetHeight < self.shower.offsetHeight){
					...
					//console.log("end"+this.ends);
					this.dis = this.ends - mover.starts;
					if (this.dis < (0-mover.can_move_top)) this.dis = 0-mover.can_move_top;
					if (this.dis >= mover.can_move_bottom){
						 this.dis = mover.can_move_bottom;
						 if(mouse_flag==0){
							setTimeout(self.loading_left(),1000);
							mouse_flag=1;
						 }
					}
		...
		if (self.container.offsetHeight < self.shower.offsetHeight){
			...
			if(self.scroll_bar.offsetTop>=self.scroller.offsetHeight-self.scroll_bar.offsetHeight){
				if(stop==true){
					stop=false;
					if(index<=total_page){
					setTimeout(getsearch(objs),1000);
					}
				}
			}
		}else{
	...
	//add_listen(this.container, "load", this.init);
