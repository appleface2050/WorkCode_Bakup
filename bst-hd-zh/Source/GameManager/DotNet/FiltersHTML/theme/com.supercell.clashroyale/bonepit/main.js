...
window.setconfig = function (config_json) {
    ...
    //console.log(config_json.animate);
    ...
    if (!config_json.animate) {
        ...
        if ($('#barb_video').length == 0) {
            $('.base-container').append('<video loop id="barb_video" muted preload="auto" style="object-fit:fill;position: absolute; z-index: -1; top: 0px; left: 0px; border: 0px none; padding: 0px;" autoplay="" class="gfyVid" height="100%" width="100%"><source class="mp4source" type="video/webm" src="http://cdn.bluestacks.com/public/btv/resources/filters/bonepit.webm"></video>');
...
//For Twitch Alerts
function displayFollowerAlert(name) {
    var followerName = name;
    if (followerName) {
        var img = document.createElement("img"),
            imageUrl = ("./assets/NEW2Skeleton.gif");
        img.src = imageUrl;
        document.getElementById('followerName').appendChild(img);
        $('#followerName').delay(1000).queue(function (next) {
            $(this).prepend("<h5>New <br> Follower</h5>");
            $(this).append("<p class='shake animated'>DemoUser<i></i></p>");
            next();
        });
        setTimeout(function () {
            $("p.shake, #followerName h5").slideDown(7000).hide();
...
$(document).ready(function () {
    initconfig();
    TwitchAlerts.setTwitchAlert(getParameterByName('channel'),getParameterByName('client_id'));
    var img = document.createElement("img"),
        imageUrl = ("./assets/NEW2Skeleton.gif");
    img.src = imageUrl;
    document.getElementById('followerName').appendChild(img);
    $('#followerName').delay(1500).queue(function (next) {
        $(this).prepend("<h5>New <br> Follower</h5>");
        $(this).append("<p class='shake animated'>DemoUser<i></i></p>");
        next();
    });
...
function handleChat(channel, user, message, self) {
    ...
    chatLine.appendChild(arrow);
    chatLine.appendChild(chatName);
    chatLine.appendChild(chatColon);
    chatLine.appendChild(chatMessage);
    chat.appendChild(chatLine);
    $('.chat-line').fadeIn('slow');
    $('.chat-line.dummy-chat').hide();
