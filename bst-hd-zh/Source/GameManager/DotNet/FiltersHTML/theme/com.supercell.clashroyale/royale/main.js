...
window.setconfig = function (config_json) {
    ...
    if (!config_json.animate) {
        ...
        if ($('#barb_video').length == 0) {
            $('.base-container').append('<video loop id="barb_video" muted preload="auto" style="object-fit:fill;position: absolute; z-index: -1; top: 0px; left: 0px; border: 0px none; padding: 0px;" autoplay="" class="gfyVid" height="100%" width="100%"><source class="mp4source" type="video/webm" src="http://cdn.bluestacks.com/public/btv/resources/filters/royale.webm"></video>');
...
function handleChat(channel, user, message, self) {
    var chan = dehash(channel),
        name = user.username,
        chatLineParent = document.createElement('div'),
    ...
    chatLineParent.className = 'chat-line-parent';
    ...
    if (name == chan) {
        chatName.innerHTML = name + " (streamer)";
    }
    ...
    chatLineParent.appendChild(chatLine);
    chatLineParent.appendChild(arrowdiv);
    chatLine.appendChild(chatName);
    chatLine.appendChild(chatColon);
    chatLine.appendChild(chatMessage);
    chat.appendChild(chatLineParent);
    setTimeout(function () {
        $('.chat-line').show("slide", {
            direction: "left"
        }, 1000);
    },1000);
...
function handleChatDummy(channel, message) {
    ...
    chatLineParent.className = 'dummy-chat chat-line-parent';
    chatLine.dataset.channel = channel;
