#!/usr/bin/python3
# -*- coding: utf-8 -*-
# Created by: python.exe -m py2exe filterlogs.py -W mysetup.py
...
filterlogs = Target(
    ...
    script="filterlogs.py", # path of the main script

    # Allows to specify the basename of the executable, if different from 'filterlogs'
    # dest_base = "filterlogs",

    # Icon resources:[(resource_id, path to .ico file), ...]
    # icon_resources=[(1, r"filterlogs.ico")]

    other_resources = [(RT_MANIFEST, 1, (manifest_template % dict(prog="filterlogs", level="asInvoker")).encode("utf-8")),
...
setup(name="name",
      ...
      console=['filterlogs.py','logstcdt.py','generatekeywords.py','clean.py'],

      # windows subsystem executables (no console)
      windows=['analyzelog.py'],
...
shutil.copy('keywords.xml', 'dist')
shutil.copy('logging.conf','dist')
shutil.copy('user-reference.txt', 'dist')
shutil.copy(r'bin\msvcr100.dll', 'dist')
if os.path.isdir(r'dist\bin'): shutil.rmtree(r'dist\bin')
shutil.copytree(r'bin', r'dist\bin', )
if not os.path.isdir(r'dist\logs'): os.mkdir(r'dist\logs')
shutil.make_archive('FilterLogsExe', 'zip', 'dist')
