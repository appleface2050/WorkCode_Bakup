...
namespace BlueStacks.hyperDroid.BlueStacksTV
{
	...
	public class FilterDownloader
	{
		...
		internal static FilterDownloader Instance = null;
		...
		public void IsFilterUpdateAvailable()
		{
			...
			{
				RemoveUninstalledAppsFilterIfThere();
		...
		private void RemoveUninstalledAppsFilterIfThere()
		{
			try
			{
				...
				string appList = (string)baseKey.GetValue("FilterAvailableForApps", "[]");
				baseKey.Close();
				...
				{
					string appPkg = obj[i].ToString();
					if (!installedApps.Contains(appPkg))
					{
						Logger.Info("FilterUpdateUtility: RemoveFilterAvailableForApp {0}", appPkg);
						try
						{
							FilterDownloader.RemoveFilterAvailableForApp(appPkg);
						}
						catch (Exception ex)
						{
							Logger.Error("FilterUpdateUtility: RemoveFilterAvailableForApp {0}", ex.ToString());
						}
					}
				}
			}
			...
			{
				Logger.Error("FilterUpdateUtility: RemoveUninstalledAppsFilterIfThere {0}", ex.ToString());
			}
		}
		...
		{
			lock (FilterUtility.sFilterDownloaderLock)
			{
		...
		public void SetUpdateStatus(string status)
		{
			...
			if (status.Equals("Error"))
			{
				if (mFilterDownloadProgress != null)
				{
					...
					{
						MessageBox.Show(Locale.Strings.GetLocalizedString("DOWNLOAD_ERROR_TEXT"));
		...
		public void ExecuteCallBack(bool callBackStatus)
		{
			StreamWindow.Instance.Dispatcher.Invoke(new Action(() =>
			{
				...
				if (mCallBackFunction != null)
					StreamWindow.Instance.EvaluateJS(mCallBackFunction + "('" + callBackStatus.ToString().ToLower() + "');");
				if (mReParentOBS)
		...
		private bool PopulateZipInfoList()
		{
			...
			mAppsForFilterDownload = new List<string>();
			...
			if (filterZipList != null)
			{
				...
				for (int i = 0; i < obj.Length; i++)
				{
					...
					if (!obj[i]["appPkg"].IsNull)
					{
						...
						if (!mAppsForFilterDownload.Contains(filterZipInfo.appPkg))
							mAppsForFilterDownload.Add(filterZipInfo.appPkg);
					}
		...
		private void KillOBSAndExtractUpdates()
		{
					...
					StreamManager.Instance.mIsStreaming)
			{
				StreamWindow.Instance.Dispatcher.Invoke(new Action(() =>
				{
					DialogResult dialogResult = MessageBox.Show(Locale.Strings.GetLocalizedString("STOP_STREAMING_REQUIRED"),
					Locale.Strings.GetLocalizedString("FILTER_UPDATE_TITLE"), MessageBoxButtons.YesNo);
		...
		private void SuccessfullyCloseUI()
		{
			StreamWindow.Instance.Dispatcher.Invoke(new Action(() =>
			{
				...
				if (mCallBackFunction != null)
					StreamWindow.Instance.EvaluateJS(mCallBackFunction + "('true');");
				mCallBackFunction = null;
		...
		private void UpdateAppList()
		{
			...
			List<string> installedApps = mAppsForFilterDownload;
					...
					if (cloudthemes.Contains(appPkg))
					{
						...
						foreach (string theme in cloudthemes[appPkg].Names)
						{
							...
							if (shouldUpdate)
							{
								...
								if (!mAppsForFilterDownload.Contains(zip.appPkg))
									mAppsForFilterDownload.Add(zip.appPkg);
							}
			...
			if (mFilterDownloadProgress != null)
			{
				...
				if (status.Equals("Downloading"))
				{
					if (mDownloadCount > mFilterZipInfo.Count)
						text = Locale.Strings.GetLocalizedString("DOWNLOADED_TEXT");
					else
						text = String.Format(Locale.Strings.GetLocalizedString("DOWNLOADING_TEXT"), mDownloadCount, mFilterZipInfo.Count);
				}
				else if (status.Equals("Downloaded"))
				{
					text = Locale.Strings.GetLocalizedString("DOWNLOADED_TEXT");
					...
					text = Locale.Strings.GetLocalizedString("APPLYING_TEXT");
