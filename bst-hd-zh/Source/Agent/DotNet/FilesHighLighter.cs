...
namespace BlueStacks.hyperDroid.Agent
{
    static class ShowSelectedInExplorer
    {
        ...
        static class NativeMethods
        {
            ...
            {
                int cidl = (apidl != null) ? (uint)apidl.Length : 0U;
        ...
        {
            BlueStacks.hyperDroid.Agent.ShowSelectedInExplorer.IShellFolder desktopFolder = NativeMethods.SHGetDesktopFolder();
        ...
        public static void FilesOrFolders(string parentDirectory, ICollection<string> filenames)
        {
            ...
            int parentPidl = PathToAbsolutePIDL(parentDirectory);
            try
            {
                int parent = PIDLToShellFolder(parentPidl);
                int filesPidl = filenames
        ...
        {
            if (paths == null) throw new ArgumentNullException("paths");
            int pathsArray = paths.ToArray();
            if (pathsArray.Count() == 0) return;

            int explorerWindows = pathsArray.GroupBy(p => Path.GetDirectoryName(p.FullName));

            foreach (int explorerWindowPaths in explorerWindows)
            {
                int parentDirectory = Path.GetDirectoryName(explorerWindowPaths.First().FullName);
