...
#region Public properties
		...
		[Category("Appearance"), RefreshProperties(RefreshProperties.All)]
			...
			{
				get { return sIsFullScreen; }
				set { sIsFullScreen = value; }
			}
		...
		public override Rectangle DisplayRectangle {
			get {
				...
				if (this._Style == TabStyle.None) {
					...
					tabStripHeight = (itemHeight * this.RowCount);

					if (this.FullScreen == true)
						return new Rectangle(0, tabStripHeight, Width, Height - tabStripHeight);

					Rectangle rect = new Rectangle(1, tabStripHeight, Width - 2, Height - tabStripHeight - 1);
					switch (this.Alignment) {
						case TabAlignment.Top:
							rect = new Rectangle(1, tabStripHeight, Width - 2, Height - tabStripHeight - 1);
							break;
						case TabAlignment.Bottom:
							rect = new Rectangle(1, 1, Width - 2, Height - tabStripHeight - 1);
							break;
						case TabAlignment.Left:
							rect = new Rectangle(tabStripHeight+1, 1, Width - tabStripHeight - 2, Height - 2);
							break;
						case TabAlignment.Right:
							rect = new Rectangle(1, 1, Width - tabStripHeight - 2, Height - 2);
							break;
					}
					return rect;
				}
...
#region	Base class event processing
		...
		protected override void OnDeselecting(TabControlCancelEventArgs e)
		{
			...
			if (this.GetTabCloserRect(this.ActiveIndex).Contains(this.MousePosition))
			{
				if (SelectedIndex != this.ActiveIndex)
				{
					e.Cancel = true;
				}
			}
...
#region	Basic drawing methods
		...
		protected override void OnPaint(PaintEventArgs e){
			...
			/*
			 * We need to paint tabbar on every resize event so commenting this code
		...
		private void DrawTabPage(int index, Graphics graphics)
		{
			...
			using (GraphicsPath tabPageBorderPath = this.GetTabPageBorder(index, graphics)) {
				...
				*/
				...
				//	Paint the border
				//removing as we donot need border anymore
		...
		private void DrawTabText(int index, Graphics graphics)
		{
			...
			/*
			   Rectangle shadowRect = new Rectangle(tabBounds.Left + 16, tabBounds.Top + 6,
			   tabBounds.Width - 16, tabBounds.Height - 6);

			   Brush shadowBrush = new SolidBrush(Color.Transparent);
			   */
			if (this.SelectedIndex == index)
			{
				...
				{
					/*
					   graphics.DrawString(this.TabPages[index].Text, this.Font, shadowBrush, shadowRect,
...
#region Tab borders and bounds properties
		...
		{
			return this._StyleProvider.GetConstantTabWidth();
		}
