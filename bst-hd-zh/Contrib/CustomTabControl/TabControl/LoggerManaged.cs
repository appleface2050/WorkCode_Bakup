...
namespace System.Windows.Forms {
				...
				String msg)
		{
			...
			if (prio == HDLOG_PRIORITY_FATAL)
				level = Logger1.LOG_LEVEL_FATAL;

			else if (prio == HDLOG_PRIORITY_ERROR)
				level = Logger1.LOG_LEVEL_ERROR;

			else if (prio == HDLOG_PRIORITY_WARNING)
				level = Logger1.LOG_LEVEL_WARNING;

			else if (prio == HDLOG_PRIORITY_INFO)
				level = Logger1.LOG_LEVEL_INFO;

			else if (prio == HDLOG_PRIORITY_DEBUG)
				level = Logger1.LOG_LEVEL_DEBUG;

			Logger1.Print(level, tag, "{0:X8}: {1}", tid, msg);
		}
			...
			)
		{
		    ...
		    Logger1.s_HdLoggerCallback = new HdLoggerCallback(HdLogger);
