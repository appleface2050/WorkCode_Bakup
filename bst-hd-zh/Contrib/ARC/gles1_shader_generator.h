...
struct ShaderConfig {
  ...
  TextureConfig texture[PGA_MAX_TEXTURE_UNITS];
  LightConfig light[PGA_MAX_LIGHTS];
  GLenum mode;
  GLenum fog_mode;
  GLenum alpha_func;
  GLuint vertex_units;
  bool enable_clip_plane[PGA_MAX_CLIP_PLANES];
...
void GenerateVertexShader(const ShaderConfig& cfg, std::string& buffer);

void GenerateFragmentShader(const ShaderConfig& cfg, std::string& buffer);
