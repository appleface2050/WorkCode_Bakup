...
#include "gles1_shader_generator.h"
...
#ifdef _WIN32
    size_t len = _vsnprintf(buf, 1024, format, args);
#else
    size_t len = vsnprintf(buf, 1024, format, args);
#endif
    va_end(args);
...
void WriteVSHeader(const ShaderConfig& cfg, StringBuilder* b) {
  ...
  if (cfg.enable_lighting) {
    b->Format("#define NUM_LIGHTS %d", PGA_MAX_LIGHTS);
  ...
  if (cfg.any_clip_planes_enabled) {
    b->Format("#define NUM_CLIP_PLANES %d", PGA_MAX_CLIP_PLANES);
  ...
  if (cfg.any_texture_units_enabled) {
    ...
    for (int stage = 0; stage < PGA_MAX_TEXTURE_UNITS; ++stage) {
  ...
  if (NeedsNormal(cfg)) {
    ...
    b->Write("uniform mediump mat3 u_mv_inverse_transpose_matrix;");
  }
  ...
  //XXXSPC: we might not need palette matrix
  if (cfg.enable_matrix_palette) {
    b->Format("#define NUM_PALETTE_MATRICES %d", 9 /*
...
void WriteFSHeader(const ShaderConfig& cfg, StringBuilder* b) {
  ...
  if (cfg.any_texture_units_enabled) {
    ...
    for (GLint stage = 0; stage < PGA_MAX_TEXTURE_UNITS; ++stage) {
...
void WriteVSComputeClipPlaneDistances(const ShaderConfig& cfg,
  ...
  for (int i = 0; i < PGA_MAX_CLIP_PLANES; ++i) {
...
void WriteVSComputeLighting(const ShaderConfig& cfg, StringBuilder* b) {
  if (cfg.enable_lighting) {
    ...
    for (int i = 0; i < PGA_MAX_LIGHTS; ++i) {
...
void WriteVSPositionAndNormal(const ShaderConfig& cfg, StringBuilder* b) {
  ...
  if (cfg.enable_matrix_palette) {
    ...
    if (NeedsNormal(cfg)) {
      b->Write("normal = u_mv_inverse_transpose_matrix * a_normal;");
    }
  }
...
void GenerateVertexShader(const ShaderConfig& config, std::string& buf) {
...
void GenerateFragmentShader(const ShaderConfig& config, std::string& buf) {
  StringBuilder buffer;
