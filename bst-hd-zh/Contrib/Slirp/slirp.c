...
static int get_dns_addr(struct in_addr *pdns_addr)
{
    ...
    if ((ret = GetNetworkParams(FixedInfo, &BufLen)) != ERROR_SUCCESS) {
        DEBUG_ARG("GetNetworkParams failed. ret = %08x\n", (u_int)ret );
        GlobalFree(FixedInfo);
        return -1;
    }
    ...
    *pdns_addr = tmp_addr;
#if 0
    DEBUG_CALL ( "DNS Servers:\n" );
    DEBUG_ARG ( "DNS Addr:%s\n", pIPAddr->IpAddress.String );
...
//Get DNS Cached DNS address, which we keep refreshing after 30 seconds..
void set_cached_dns_addr(struct in_addr addr)
{
	dns_addr = addr;
}
...
{
#ifdef DEBUG
    LoggerPrint("Initialize debugging");
    debug_init(DEBUG_DEFAULT);
#endif
...
void slirp_select_poll(fd_set *readfds, fd_set *writefds, fd_set *xfds)
{
	...
	if (link_up) {
		...
		for (so = tcb.so_next; so != &tcb; so = so_next) {
			...
			if (FD_ISSET(so->s, writefds)) {
			  ...
			  if (so->so_state & SS_ISFCONNECTING) {
			    ...
			    if (ret < 0) {
			      ...
			      if (errno == EAGAIN ||
				  errno == SLIRP_EWOULDBLOCK ||
				  errno == SLIRP_EINPROGRESS ||
				  errno == SLIRP_ENOTCONN)
				continue;
			 ...
			 *
			 * Use getsockopt() to check for SO_ERROR instead
			 * of the recv()/send() dance commented out below.  
			 * This approach involves less logic and is known
			 * to work on Windows.
	 	 	 */
			...
			if (so->so_state & SS_ISFCONNECTING) {
...
#ifdef __APPLE__
				/*
				 * Something is seriously broken here.  We
				 * have to do something before our call to
				 * getsockopt() or we mysteriously crash
				 * after returning.
				 */
				sleep(0);
#endif
				...
				if (getsockopt(so->s, SOL_SOCKET, SO_ERROR,
				    (char *)&err, &errLen) == -1) {
#ifdef _WIN32
					LoggerPrintWin32(WSAGetLastError(),
					    "Cannot get connect error code");
#else
					LoggerPrintErrno(errno,
					    "Cannot get connect error code");
#endif
					continue;
				}
				...
				if (err != 0) {
				DEBUG_ARG("getsockopt Connect error  %d", err);
					so->so_state = SS_NOFDREF;
					tcp_input((struct mbuf *)NULL,
					    sizeof(struct ip), so);
				}
			}
                ...
                int listen_on_wildcard)
{
    ...
    if (is_udp) {
        if (!udp_listen(htons(host_port), guest_addr.s_addr,
                        htons(guest_port), 0, listen_on_wildcard))
            return -1;
    } else {
