	...
	int n;
{
	...
	LoggerPrint("PACKET DUMPED:");

	for(j = 0; j < n; j++) {

		for(k = 0; k < 6; k++) {
			res = sprintf(line_ptr, "%02x ", *pptr++);
			line_ptr += res;
		}
...
sockstats()
{
	...
	for (so = tcb.so_next; so != &tcb; so = so->so_next) {
		...
		strncpy(laddr, inet_ntoa(so->so_laddr), sizeof(laddr));
		strncpy(faddr, inet_ntoa(so->so_faddr), sizeof(faddr));

		lprint("%s %4d  %15s %5d  %15s %5d %5d %5d\r\n",
				buff, so->s,
				laddr, ntohs(so->so_lport),
				faddr, ntohs(so->so_fport),
