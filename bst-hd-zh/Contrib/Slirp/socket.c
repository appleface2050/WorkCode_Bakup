	...
	struct socket *so;
{
	...
	if (so->so_type == IPPROTO_ICMP) {   /* This is a "ping" reply */
	  ...
	  if(len == -1 || len == 0) {
	    ...
	    if(errno == SLIRP_EHOSTUNREACH) code=ICMP_UNREACH_HOST;
	    else if(errno == SLIRP_ENETUNREACH) code=ICMP_UNREACH_NET;
 ...
 */
struct socket *
solisten(port, laddr, lport, flags, listen_on_wildcard)
	u_int port;
	u_int32_t laddr;
	u_int lport;
	int flags;
	int listen_on_wildcard;
{
	...
	if (((s = socket(AF_INET,SOCK_STREAM,0)) < 0) ||
	    (setsockopt(s,SOL_SOCKET,SO_EXCLUSIVEADDRUSE,(char *)&opt,sizeof(int)) < 0) ||
