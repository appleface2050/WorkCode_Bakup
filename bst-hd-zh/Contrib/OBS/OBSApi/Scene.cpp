...
SceneItem* Scene::InsertImageSource(UINT pos, XElement *sourceElement)
{
    ...
    /*if (className.Compare(L"WindowCaptureSource1"))
    {
        XElement *data = sourceElement->GetElement(TEXT("data"));
        cx  = data->GetFloat(TEXT("captureCX"));
        cy  = data->GetFloat(TEXT("captureCY"));
    }
    else
    {*/
    cx = sourceElement->GetFloat(TEXT("cx"), 100);
    cy = sourceElement->GetFloat(TEXT("cy"), 100);
    //}

/*
    FILE* f = fopen("foooooooooooooo", "w");
    fprintf(f, "capture cx: %f cy: %f\n", captureCX, captureCY);
    fclose(f);
*/
...
/*void Scene::ExecuteJS(std::wstring lines)
{
    for(UINT i=0; i<sceneItems.Num(); i++)
    {
        SceneItem *item = sceneItems[i];
        if(item->source && item->bRender)
			item->source->ExecuteJS(lines);
    }
}*/
