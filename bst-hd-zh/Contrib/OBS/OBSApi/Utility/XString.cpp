...
BOOL String::FindIgnoreCase(CTSTR strContain)
{
	const TCHAR *strSrc = *this;
	assert(strSrc);

	const TCHAR *lpTemp = strSrc;

	const TCHAR *lpTempE = strSrc + slen(strSrc);
	while (lpTemp != lpTempE)
	{
		TCHAR test = *lpTemp;
		if ((test >= 'A') && (test <= 'Z'))
			test += 0x20;
		...
		BOOL success = true;
		const TCHAR *lpTemp2 = strContain;
		const TCHAR *lpTempE2 = strContain + slen(strContain);
		const TCHAR *temp = lpTemp;
		while (lpTemp2 != lpTempE2)
		{
			TCHAR test2 = *lpTemp2;
			if ((test2 >= 'A') && (test2 <= 'Z'))
				test2 += 0x20;
