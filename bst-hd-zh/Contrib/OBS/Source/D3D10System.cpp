...
D3D10System::D3D10System()
{
	...
	{
		String cardName = outputs.devices[i].strDevice;
		TCHAR *strSrc = cardName;
		Log(TEXT("Looking For graphics card  %s"), strSrc);
		if (!cardName.FindIgnoreCase(TEXT("Intel")))
		{
			Log(TEXT("Selected Graphics card For %s"), strSrc);
			adapterID = i;
		}
	}
