...
DWORD WINAPI RTMPPublisher::CreateConnectionThread(RTMPPublisher *publisher)
{
	...
	Log(TEXT("url : %s"), strURL);
	Log(TEXT("play path : %s"), strPlayPath);
    ...
    if(!bSuccess)
    {
        ...
        {
            if (failReason.IsValid() && failReason.FindIgnoreCase(L"Could not access the specified channel or stream key.  This could be because the key/channel is invalid, or because the server still thinks you are logged in."))
            {
                if (failReason.FindIgnoreCase(L"RTMPSockBuf_Fill, remote host closed connection"))
                {
                    Log(TEXT("found the given could not access the specified channel"));
                    errorStr = TEXT("Error=AccessDenied: ");
                    errorStr += failReason;
                    HTTPPostString(errorStr.CreateUTF8String());
                }
				else
                {
                    errorStr = TEXT("Error=AccessDenied_ConnectionIssue: ");
                    errorStr += failReason;
                    HTTPPostString(errorStr.CreateUTF8String());
                }
            }
            else if (failReason.IsValid() && failReason.FindIgnoreCase(L"Could not connect to server"))
            {
                errorStr = TEXT("Error=ConnectServerError: ");
                errorStr += failReason;
                HTTPPostString(errorStr.CreateUTF8String());
            }
            else
            {
                errorStr = TEXT("Error=ConnectionIssue: ");
                if (failReason.IsValid())
                     errorStr += failReason;
                HTTPPostString(errorStr.CreateUTF8String());
            }
    ...
    else
    {
        ...
        HTTPPostString("Error=ConnectionSuccessfull");
    }
    ...
    for (;;)
    {
        ...
        if (status == WAIT_OBJECT_0)
        {
            ...
            if (networkEvents.lNetworkEvents & FD_CLOSE)
            {
				...
				{
					String failReason = FormattedString(TEXT("RTMPPublisher::SocketLoop: Aborting due to FD_CLOSE, error %d"), networkEvents.iErrorCode[FD_CLOSE_BIT]);
					Log(failReason.Array());
					String errorStr = TEXT("Error=ConnectionIssue: ");
					errorStr += failReason;
					HTTPPostString(errorStr.CreateUTF8String());
				}
                FatalSocketShutdown ();
