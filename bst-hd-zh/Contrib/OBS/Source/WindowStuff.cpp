    ...
    switch(message)
    {
        case WM_COMMAND:
            switch(LOWORD(wParam))
            {
                ...
                case ID_STARTRECORD:
                    App->StartRecordingExt();
                    break;

                case ID_STOPRECORD:
                    App->StopRecordingExt(HIWORD(wParam));
                    break;

                case ID_STARTREPLAYBUFFER:
                    App->StartReplayBuffer();
                    break;

                case ID_STOPREPLAYBUFFER:
...
//                    App->streamReport.Clear();
            ...
            else if(wParam == 0)
            {
                ...
                //SetTimer(hwnd, 1, 1000 * App->reconnectTimeout, NULL);
...
//                    App->streamReport.Clear();
            ...
            else
            {
                ...
                SetTimer(hwnd, 2, 1000 * App->reconnectTimeout, NULL);
/*
                if (!App->reconnectTimeout)
                {
                    //fire immediately
...
LRESULT CALLBACK OBS::RenderFrameProc(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    else if(message == WM_LBUTTONUP || message == WM_RBUTTONUP)
    {
        ...
        {
            if (message == WM_RBUTTONUP && !bDebugMode)
                 return DefWindowProc(hwnd, message, wParam, lParam);
