...
public:
    ...
    bool EnableWebcam(int width, int height, int position);
    void ExecuteJS(std::wstring lines);
    void RenderCLRBrowser(bool render);
    void SetCLRBrowserConfig(String url, int width, int height);
    void DisableWebcam();
    void MoveWebcamToEdge(int horizontal, int vertical);
    void SaveReplayBufferExt();
    void ReplayBufferSaved();
    void SetFrontendPosition(int width, int height, int x, int y);
    void SetSourcePosition(String source, int x, int y, int width, int height);
    void ResetToOriginalScene();
    void SetRender(String sourceStr, int render);
    void SetCameraPosition(int width, int height, int x, int y, int cameraActualWidth, int cameraActualHeight, int isPercentage, int render);
    void SetCLRStreamConfig(int width, int height);
    void SetSourceOrder(String sources);

    void ToggleRecording();
    void StartRecordingExt();
    void StopRecordingExt(int immediate);
    void ResetFLVStreamRecording();
    void ResizeWindow(bool bRedrawRenderFrame);
    void SetFullscreenMode(bool fullscreen);
    bool GetStreamingStatus() { return bStreaming; }
    bool GetReconnectingStatus() { return bReconnecting; }
    String GetStreamReport() { return streamReport; }
    void ClearStreamReport() { streamReport.Clear(); }
