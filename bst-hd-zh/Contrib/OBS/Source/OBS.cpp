...
OBS::OBS()
{
    ...
    if (bDebugMode)
        ...
        hwndRenderMessage = CreateWindow(TEXT("STATIC"), Str(""),
            WS_CHILDWINDOW|WS_CLIPSIBLINGS|SS_CENTER,
            0, 0, 0, 0, hwndRenderFrame, NULL, hinstMain, NULL);
...
bool FillList(GUID matchGUID, StringList *deviceList, StringList *deviceIDList)
{
    Log(TEXT("Started FillList"));
    ...
    if(FAILED(err))
    {
        Log(TEXT("FillOutListDevices: CoCreateInstance for the device enum failed, result = %08lX"), err);
        return false;
    }
    ...
    if(FAILED(err))
    {
        Log(TEXT("FillOutListDevices: deviceEnum->CreateClassEnumerator failed, result = %08lX"), err);
        deviceEnum->Release();
        return false;
    }
    ...
    {
        Log(TEXT("FillOutListDevices: CreateClassEnumerator returned S_FALSE. No devices found."));
        return false;
    }
...
void OBS::ExecuteJS(std::wstring lines)
{
    if (sceneElement)
    {
        HWND hwndSources = GetDlgItem(hwndMain, ID_SOURCES);
        ...
        if (sources)
        {
            for (int i = numSources - 1; i >= 0; i--)
            {
                ...
                if (className.Compare(L"CLRBrowserSource"))
                {
					bool render = (sourceElement->GetInt(TEXT("render")) > 0);
                    if (scene && i < scene->NumSceneItems() && render)
                    {
                        SceneItem *sceneItem = scene->GetSceneItem(i);
                        //sceneItem->source->ExecuteJS(lines);
                    }
                }
            }
        }
...
void OBS::RenderCLRBrowser(bool render)
{
    if (sceneElement)
    {
        HWND hwndSources = GetDlgItem(hwndMain, ID_SOURCES);
        ...
        if (sources)
        {
            for (int i = numSources - 1; i >= 0; i--)
            {
                ...
                if (className.Compare(L"CLRBrowserSource"))
                {
                    if ((sourceElement->GetInt(TEXT("render"), 0) > 0) == render)
                        return;

					if (render)
						sourceElement->SetInt(TEXT("render"), 1);
					else
						sourceElement->SetInt(TEXT("render"), 0);
					
					if (scene && i < scene->NumSceneItems())
                    {
                        SceneItem *sceneItem = scene->GetSceneItem(i);
                        sceneItem->bRender = render;
                    ...
                    if (checked != render)
                    {
                        App->bChangingSources = render;
                        ListView_SetCheckState(hwndSources, i, render);
                        App->bChangingSources = render;

                        ReportSourceChanged(sourceElement->GetName(), sourceElement);
                    }
...
void OBS::SetCLRBrowserConfig(String url, int width, int height)
{
	if (sceneElement)
	{
        XElement *sources = sceneElement->GetElement(TEXT("sources"));
		...
		if (sources)
		{
			for (int i = numSources - 1; i >= 0; i--)
			{
				...
				if (className.Compare(L"CLRBrowserSource"))
				{
					...
					if (!url.Compare(L""))
						data->SetString(TEXT("url"), url);
					if (width != 0)
						data->SetInt(TEXT("width"), width);
					if (height != 0)
						data->SetInt(TEXT("height"), height);
...
void OBS::SetSourcePosition(String sourceStr, int x, int y, int width, int height)
{
	if (sceneElement)
	{
		XElement *sources = sceneElement->GetElement(TEXT("sources"));
		...
		if (sources)
		{
			XElement *sourceElement = sources->GetElement(sourceStr);

			if (sourceElement == NULL)
				return;

			XElement *data = sourceElement->GetElement(TEXT("data"));
			int windowWidth = AppConfig->GetInt(TEXT("Video"), TEXT("WindowWidth"));
			int windowHeight = AppConfig->GetInt(TEXT("Video"), TEXT("WindowHeight"));

			//wont work for webcam
			if (!sourceStr.Compare(L"BlueStacksWebcam"))
			{
				int cx = (int)round(windowWidth * width * 1.0f / 100);
				int cy = (int)round(windowHeight * height * 1.0f  / 100);
				int x_val = (int)round(windowWidth * x * 1.0f / 100);
				int y_val = (int)round(windowHeight * y * 1.0f / 100);

				sourceElement->SetInt(TEXT("cx"), cx);
				sourceElement->SetInt(TEXT("cy"), cy);
				sourceElement->SetInt(TEXT("x"), x_val);
				sourceElement->SetInt(TEXT("y"), y_val);

				if (scene && scene->GetSceneItem(sourceStr) != NULL)
				{
					SceneItem *sceneItem = scene->GetSceneItem(sourceStr);
					sceneItem->pos.x = sourceElement->GetInt(TEXT("x"));
					sceneItem->pos.y = sourceElement->GetInt(TEXT("y"));
					sceneItem->size.x = sourceElement->GetInt(TEXT("cx"));
					sceneItem->size.y = sourceElement->GetInt(TEXT("cy"));
				}
			}
			App->scenesConfig.Save();
			App->scenesConfig.SaveTo(String() << lpAppDataPath << "\\scenes.xconfig");
...
void OBS::SetFrontendPosition(int width, int height, int x, int y)
{
	...
	if (sceneElement)
	{
		...
		if (sources)
		{
			for (int i = numSources - 1; i >= 0; i--)
			{
				...
				String className = sourceElement->GetString(TEXT("class"));
				if (className.Compare(L"WindowCaptureSource"))
				{
					XElement *data = sourceElement->GetElement(TEXT("data"));

					int windowWidth = AppConfig->GetInt(TEXT("Video"), TEXT("WindowWidth"));
					int windowHeight = AppConfig->GetInt(TEXT("Video"), TEXT("WindowHeight"));
					//to save unnecessary update
					/*if (data->GetInt(TEXT("captureX"), 0) == x
						&& data->GetInt(TEXT("captureY"), 0) == y
						&& data->GetInt(TEXT("captureCX"), 0) == width
						&& data->GetInt(TEXT("captureCY"), 0) == height)
						return;
						*/
					if (scene && i < scene->NumSceneItems())
					{
					...
					data->SetInt(TEXT("captureX"), x);
					data->SetInt(TEXT("captureY"), y);
					data->SetInt(TEXT("captureCX"), width);
					data->SetInt(TEXT("captureCY"), height);
					
					//hack for layout theme not updating cx, cy
					if (windowWidth != windowHeight)
					{
						sourceElement->SetInt(TEXT("cx"), width);
						sourceElement->SetInt(TEXT("cy"), height);
						sourceElement->SetInt(TEXT("x"), ((windowWidth - width) / 2));
						sourceElement->SetInt(TEXT("y"), 0);
					}
					...
					if (scene && i < scene->NumSceneItems())
					{
						...
						sceneItem->bRender = true;
						sceneItem->SetRender(true);
					}
					App->scenesConfig.Save();
					App->scenesConfig.SaveTo(String() << lpAppDataPath << "\\scenes.xconfig");
				}
...
void OBS::ResetToOriginalScene()
{
	StringList* requiredSourceOrder = new StringList();
	requiredSourceOrder->Add("watermarkFB");
	requiredSourceOrder->Add("watermarkGif");
	requiredSourceOrder->Add("watermark");
	requiredSourceOrder->Add("BlueStacksWebcam");
	requiredSourceOrder->Add("BlueStacks");
	requiredSourceOrder->Add("CLR Browser");

	SetSourceOrder(*requiredSourceOrder);

	SetRender("watermarkGif", 1);
	SetRender("watermark", 1);
	SetRender("watermarkFB", 0);

	App->scenesConfig.Save();
	App->scenesConfig.SaveTo(String() << lpAppDataPath << "\\scenes.xconfig");
}

void OBS::SetSourceOrder(String sourcesString)
{
	StringList sourcesList;
	sourcesString.GetTokenList(sourcesList, ',', false);

	StringList* availableSourcesList = new StringList();
	...
	if (sceneElement)
	{
		...
		if (sources)
		{
			...
			{
				String name = sources->GetElementByID(i)->GetName();
				if (sourcesList.FindValueIndex(name) != -1)
					availableSourcesList->Add(name);
			}
		}
	}
	...
	for (int i = 0; i < sourcesList.Num(); i++)
	{
		if (availableSourcesList->FindValueIndex(sourcesList.GetElement(i)) != -1)
			requiredSourceOrder->Add(sourcesList.GetElement(i));
	}
...
void OBS::SetRender(String sourceStr, int render)
{
	if (sceneElement)
	{
		XElement *sources = sceneElement->GetElement(TEXT("sources"));
		...
		if (sources)
		{
			...
			if (scene && scene->GetSceneItem(sourceStr) != NULL)
			{
				...
				if (sourceElement->GetInt(TEXT("render"), 0) > 0 && render == 0)
				{
					sceneItem->bRender = false;
					sceneItem->SetRender(false);
				}
				else if (sourceElement->GetInt(TEXT("render"), 0) == 0 && render > 0)
				{
					sceneItem->bRender = true;
					sceneItem->SetRender(true);
				}
				...
				else if (!checked && render > 0)
				{
					App->bChangingSources = true;
					ListView_SetCheckState(hwndSources, i, true);
					App->bChangingSources = false;
			...
			sourceElement->SetInt(TEXT("render"), render);
			App->scenesConfig.Save();
			App->scenesConfig.SaveTo(String() << lpAppDataPath << "\\scenes.xconfig");
		}
	}
...
void OBS::SetCameraPosition(int width, int height, int x, int y, int cameraActualWidth, int cameraActualHeight, int isPercentage,
	int render)
{
    StringList deviceNameList;
    StringList deviceIDList;
    bool ret = FillList(CLSID_VideoInputDeviceCategory, &deviceNameList, &deviceIDList);
    Log(TEXT("FillList returned: %d"), ret);
    UINT count = deviceNameList.Num();
    Log(TEXT("num of camera : %u"), count);
    ...
    if (count > 0)
    {
        ...
        Log(TEXT("device name : %s"), deviceName);
        Log(TEXT("device id : %s"), deviceID);
    }
    ...
    if (sceneElement)
    {
        ...
        if (sources)
        {
            for (int i = numSources - 1; i >= 0; i--)
            {
                ...
                int windowHeight = AppConfig->GetInt(TEXT("Video"), TEXT("WindowHeight"));
                String className = sourceElement->GetString(TEXT("class"));
                if (className.Compare(L"DeviceCapture"))
                {
                    if (cameraActualWidth == -1)
                    {
                        cameraActualWidth = windowWidth;
                        if (windowWidth == 320 || windowWidth == 640)
                            cameraActualHeight = cameraActualWidth * 3 / 4;
                        else
                            cameraActualHeight = cameraActualWidth * 720 / 1280;
                    }

                    if (cameraActualHeight == -1)
                    {
                        cameraActualHeight = windowHeight;
                        if (windowHeight == 240 || windowHeight == 480)
                             cameraActualWidth = cameraActualHeight * 4 / 3;
                        else
                             cameraActualWidth = cameraActualHeight * 1280 / 720;
                    }
					...
					if (sourceElement->GetInt(TEXT("render"), 0) != render)
					{
						if (scene && i < scene->NumSceneItems())
						{
							SceneItem *sceneItem = scene->GetSceneItem(i);
                    ...
                    bool takeWidthAsOriginal = false;
                    bool dontStretchImage = ((cameraActualWidth - width) >= 0 && (cameraActualHeight - height) >= 0);

                    if (ratio > (4.0 / 3))
                        takeWidthAsOriginal = true;

                    XElement *data = sourceElement->GetElement(TEXT("data"));

                    float widthCrop = 0.0, heightCrop = 0.0;
                    if (dontStretchImage)
                    {
                        widthCrop = ((cameraActualWidth - width) * 1.0) / 2;
                        heightCrop = ((cameraActualHeight - height) * 1.0) / 2;

                        width = cameraActualWidth;
                        height = cameraActualHeight;
                        x = x - (int)widthCrop;
                        y = y - (int)heightCrop;
                    }
                    else
                    {
                        if (takeWidthAsOriginal)
                        {
                            float expectedHeight = (width * 3.0 / 4);
                            heightCrop = ((((expectedHeight - (float)height) * 1.0)) / 2) * ((float)cameraActualHeight/expectedHeight);

                            y = y - (int)((((expectedHeight - (float)height) * 1.0)) / 2);
                            height = (int)expectedHeight;
                        }
                        else
						{
                            float expectedWidth = (int)(height * 4.0 / 3);
                            widthCrop = ((((expectedWidth - (float)width) * 1.0)) / 2) * ((float)cameraActualWidth/expectedWidth);

                            x = x - (int)((((expectedWidth - (float)width) * 1.0)) / 2);
                            width = (int)expectedWidth;
                        }
                    }

                    Log(TEXT("width x height %d, %d"), cameraActualWidth, cameraActualHeight);
                    data->SetString(TEXT("device"), deviceName);
                    data->SetString(TEXT("deviceName"), deviceName);
                    data->SetString(TEXT("deviceID"), deviceID);
                    if (data->GetInt(TEXT("resolutionWidth")) != cameraActualWidth
                        || data->GetInt(TEXT("resolutionHeight")) != cameraActualHeight)
                    {
                        if (scene && i < scene->NumSceneItems())
                        {
                            SceneItem *sceneItem = scene->GetSceneItem(i);
                        ...
                        data->SetInt(TEXT("resolutionWidth"), cameraActualWidth);
                        data->SetInt(TEXT("resolutionHeight"), cameraActualHeight);
                        data->SetInt(TEXT("scaleFactor_x"), cameraActualWidth);
                        data->SetInt(TEXT("scaleFactor_y"), cameraActualHeight);
                        if (scene && i < scene->NumSceneItems())
                        {
                            SceneItem *sceneItem = scene->GetSceneItem(i);
                            sceneItem->bRender = true;
                    ...
                    sourceElement->SetInt(TEXT("cx"), width);
                    sourceElement->SetInt(TEXT("cy"), height);
                    sourceElement->SetInt(TEXT("x"), x);
                    sourceElement->SetInt(TEXT("y"), y);
                    sourceElement->SetFloat(TEXT("crop.left"), widthCrop);
                    sourceElement->SetFloat(TEXT("crop.top"), heightCrop);
                    sourceElement->SetFloat(TEXT("crop.right"), widthCrop);
                    sourceElement->SetFloat(TEXT("crop.bottom"), heightCrop);

                    if (sourceElement->GetInt(TEXT("render"), 0) > 0)
                    {
                        if (scene && i < scene->NumSceneItems())
                        {
                            SceneItem *sceneItem = scene->GetSceneItem(i);
                            ...
                            sceneItem->size.x = sourceElement->GetInt(TEXT("cx"));
                            sceneItem->size.y = sourceElement->GetInt(TEXT("cy"));
                            sceneItem->crop.x = sourceElement->GetFloat(TEXT("crop.left"));
                            sceneItem->crop.y = sourceElement->GetFloat(TEXT("crop.top"));
                            sceneItem->crop.w = sourceElement->GetFloat(TEXT("crop.right"));
                            sceneItem->crop.z = sourceElement->GetFloat(TEXT("crop.bottom"));
                        }
                    } 
...
void OBS::SetCLRStreamConfig(int width, int height)
{
	if (sceneElement)
	{
		XElement *sources = sceneElement->GetElement(TEXT("sources"));
 ...
 * 0 - default position just enable it
 * 1 - top left
 * 2 - top right
 * 3 - bottom left
 * 4 - bottom right
 */
bool OBS::EnableWebcam(int width, int height, int position)
{
    StringList deviceNameList;
    StringList deviceIDList;
    bool ret = FillList(CLSID_VideoInputDeviceCategory, &deviceNameList, &deviceIDList);
    ...
    if(sceneElement && deviceName.IsValid())
    {
        HWND hwndSources = GetDlgItem(hwndMain, ID_SOURCES);
        XElement *sources = sceneElement->GetElement(TEXT("sources"));
        if(sources)
	    ...
	    int windowHeight = AppConfig->GetInt(TEXT("Video"), TEXT("WindowHeight"));
            ...
            for(int i=numSources-1; i>=0; i--)
            {
                ...
                String className = sourceElement->GetString(TEXT("class"));
				Log(TEXT("class name : %s"), className);
                if (className.Compare(L"DeviceCapture") || className.Compare(L"DeviceCaptureTemp"))
                {
                    ...
                    Log(TEXT("windowWidth = %d"), windowWidth);
                    Log(TEXT("windowHeight = %d"), windowHeight);
                    Log(TEXT("positionX = %d"), positionX);
                    Log(TEXT("positionY = %d"), positionY);

                    if (position != 0)
                    {
                        sourceElement->SetInt(TEXT("cx"), width);
                        sourceElement->SetInt(TEXT("cy"), height);
                        sourceElement->SetInt(TEXT("x"), positionX);
                        sourceElement->SetInt(TEXT("y"), positionY);
                        sourceElement->SetFloat(TEXT("crop.left"), 0.0);
                        sourceElement->SetFloat(TEXT("crop.top"), 0.0);
                        sourceElement->SetFloat(TEXT("crop.right"), 0.0);
                        sourceElement->SetFloat(TEXT("crop.bottom"), 0.0);
                    }
                    else
                        Log(TEXT("Position value is 0 so skipping position and size changes just enabling camera"));
                    ...
                    data->SetString(TEXT("deviceName"), deviceName);
                    data->SetString(TEXT("deviceID"), deviceID);
                    data->SetInt(TEXT("scaleFactor_x"), 320);
                    data->SetInt(TEXT("scaleFactor_y"), 240);
                    data->SetInt(TEXT("resolutionWidth"), 320);
                    data->SetInt(TEXT("resolutionHeight"), 240);

                    sourceElement->SetString(TEXT("class"), L"DeviceCapture");
 ...
 * bottom: 0, 1
 */
void OBS::MoveWebcamToEdge(int horizontal, int vertical)
{
    SceneItem *item = App->scene->GetSceneItem(L"BluestacksWebcam");
...
/*
        FILE* f = fopen("foo", "w");
        fprintf(f, "got args %d, %d\n", horizontal, vertical);
        fprintf(f, "got basesize %f, %f\n", baseSize.x, baseSize.y);
        fprintf(f, "got size %f, %f\n", item->size.x, item->size.y);
        fprintf(f, "got crop %f, %f\n", item->GetCrop().w, item->GetCrop().z);
        fprintf(f, "got crop %f, %f\n", item->GetCrop().x, item->GetCrop().y);
        fprintf(f, "settings pos %f, %f\n", item->pos.x, item->pos.y);
        fclose(f);
*/
    }
}
