...
void OBS::StartCapturing()
{
    if(!bRunning || (!bStreaming && (bRecording || bRecordingReplayBuffer)))
        Start();
}

void OBS::StopCapturing()
{
    if(bRunning && (bStreaming || (!bRecording && !bRecordingReplayBuffer)))
        Stop();
}

void OBS::StartRecordingExt()
{
    if (!bRecording)
    {
        if (bRunning || bStreaming)
            StartRecording(true);
        else
            Start(true);
    }
...
void OBS::StopRecordingExt(int immediate)
{
    if (bRecording)
    {
        StopRecording(immediate ? true : false);
    }
}

void OBS::SaveReplayBufferExt()
{
    if (bRunning)
    {
...
void OBS::ReplayBufferSaved()
{
    ...
    char path[30] = "replaybuffersaved";
    String* pathStr = new String(path);

    String url = *localhostStr + *pathStr;
...
bool OBS::StartRecording(bool force)
{
    ...
    bWriteToFile = networkMode == 1 || saveToFile;
...
void OBS::ResetFLVStreamRecording()
{
	if (!bRecording) return;

	auto shutdown = [this]()
	{
		AddPendingStream(fileStream.release(), [this]()
		{
			String strOutputFile = GetOutputFilename();
			fileStream.reset(CreateFileStream(strOutputFile));
			if (!fileStream)
			{
				Log(TEXT("Warning - OBSCapture::Start: Unable to create the file stream. Check the file path in Broadcast Settings."));
...
retryHookTest:
    ...
    if (OSIncompatibleModulesLoaded())
    {
        ...
        if (ret == IDABORT)
        {
            ...
            bStartingUp = false;
            HTTPPostString("Error=IncompatibleModules");
            return;
        }
        else if (ret == IDRETRY)
        {
    ...
    if (OSIncompatiblePatchesLoaded(strPatchesError))
    {
        ...
        bStartingUp = false;
        HTTPPostString("Error=IncompatiblePatches");
        return;
    }
    ...
    if (!bTestStream)
    {
        ...
        if (!foundSource)
        {
            if (OBSMessageBox(hwndMain, Str("NoSourcesFound"), NULL, MB_ICONWARNING|MB_YESNO) == IDNO)
            {
                ...
                OSLeaveMutex(hStartupShutdownMutex);
                bStartingUp = false;
                HTTPPostString("Error=NoSourcesFound");
                return;
            }
        }
    }
    ...
    if(!network)
    {
        ...
        bStartingUp = false;
        HTTPPostString("Error=NoInternet");
        return;
    }
...
retryHookTestV2:
    ...
    if(sceneElement)
    {
        ...
        if(sources)
        {
            ...
            for(UINT i=0; i<numSources; i++)
            {
                ...
                if (className.Compare(L"DeviceCaptureTemp"))
                    continue;

                if (className.Compare(L"WindowCaptureSource"))
                {
                    int startX = AppConfig->GetInt(L"Video", L"StartX");
                    int startY = AppConfig->GetInt(L"Video", L"StartY");
                    int windowWidth = AppConfig->GetInt(L"Video", L"WindowWidth");
                    int windowHeight = AppConfig->GetInt(L"Video", L"windowHeight");
                    XElement *data = sourceElement->GetElement(TEXT("data"));

                    //data->SetInt(TEXT("captureX"), startX);
                    //data->SetInt(TEXT("captureY"), startY);
                    //data->SetInt(TEXT("captureCX"), windowWidth);
                    //data->SetInt(TEXT("captureCY"), windowHeight);
                    App->scenesConfig.Save();
                    App->scenesConfig.SaveTo(String() << lpAppDataPath << "\\scenes.xconfig");
                }

                if (className.Compare(L"BitmapImageSource"))
                {
                    String name = sourceElement->GetName();
                    int windowWidth = AppConfig->GetInt(L"Video", L"WindowWidth");
                    int windowHeight = AppConfig->GetInt(L"Video", L"windowHeight");
					if (name.Compare(L"watermark") || name.Compare(L"watermarkGif"))
					{
						int watermarkWidth = (250 * windowWidth) / 960;
						int watermarkHeight = (watermarkWidth * 66) / 250;
						int watermarkXCoordinate = windowWidth - watermarkWidth;
						int watermarkYCoordinate = windowHeight - watermarkHeight;
						sourceElement->SetInt(TEXT("cx"), watermarkWidth);
						sourceElement->SetInt(TEXT("cy"), watermarkHeight);
						sourceElement->SetInt(TEXT("x"), watermarkXCoordinate);
						sourceElement->SetInt(TEXT("y"), watermarkYCoordinate);	
					}
                    else if (name.Compare(L"watermarkFB"))
                    {
						int watermarkWidth = (112 * windowWidth) / 500;
						int watermarkHeight = (watermarkWidth * 39) / 112;
						int watermarkXCoordinate = 0;
						int watermarkYCoordinate = windowHeight - watermarkHeight;
						sourceElement->SetInt(TEXT("cx"), watermarkWidth);
						sourceElement->SetInt(TEXT("cy"), watermarkHeight);
						sourceElement->SetInt(TEXT("x"), watermarkXCoordinate);
                ...
                SceneItem *item = scene->AddImageSource(sourceElement);
                if(item)
                {
    ...
    if (!videoEncoder)
    {
        ...
        HTTPPostString("Error=VideoEncoderNotIntialized");
        return;
    }
...
void OBS::ClosePendingStreams()
{
    ...
    {
        WaitForMultipleObjects(handles.size(), handles.data(), true, 15 * 1000);
