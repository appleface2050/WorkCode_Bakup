...
String HTTPPostString(LPSTR extraHeaders)
{
	char localhost[30] = "http://127.0.0.1/";
	String* localhostStr = new String(localhost);
	int port = 2885;
	char gmPath[30] = "obsstatus";
	String* pathStr = new String(gmPath);
	...
	BOOL bResults = WinHttpSendRequest(hRequest, L"content-type:application/x-www-form-urlencoded", -1, extraHeaders, strlen(extraHeaders), strlen(extraHeaders), NULL);
	...
	if (bResults && wcstoul(statusCode, NULL, 10) == 200)
	{
		CHAR buffer[16384];
		DWORD dwSize, dwOutSize;
