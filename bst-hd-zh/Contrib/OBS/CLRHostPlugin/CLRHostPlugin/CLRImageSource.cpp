...
bool CLRImageSource::Attach(CLRObjectRef &clrObjectRef, mscorlib::_Type *objectType)
{
    ...
    bstr_t executeJSMethodName("ExecuteJS");
    ...
    hr = objectType->GetMethod_6(executeJSMethodName, &executeJSMethod);
    if (FAILED(hr) || !executeJSMethod) {
        Log(TEXT("Failed to get ExecuteJS method definition of ImageSource class: 0x%08lx"), hr);
        goto errorCleanup;
    }
...
void CLRImageSource::Detach()
{
    ...
    if (executeJSMethod) {
        executeJSMethod->Release();
        executeJSMethod = nullptr;
    }
...
void CLRImageSource::ExecuteJS(std::wstring lines)
{
    if (!IsValid()) {
        Log(TEXT("CLRPlugin::ExecuteJS() no managed object attached"));
    }

    variant_t objectRef(GetObjectRef());
    variant_t valLines(lines.c_str());
    HRESULT hr;
    SAFEARRAY *args = SafeArrayCreateVector(VT_VARIANT, 0, 1);
    LONG index = 0;

    hr = SafeArrayPutElement(args, &index, &valLines);

    hr = executeJSMethod->Invoke_3(objectRef, args, nullptr);
    SafeArrayDestroy(args);

    if (FAILED(hr)) {
        Log(TEXT("CLRPlugin::ExecuteJS() failed to invoke on managed instance: 0x%08lx"), hr);
    }
